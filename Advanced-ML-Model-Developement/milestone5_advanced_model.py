...
import os
rule_based_csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../Rule-Based Engine/milestone4_output.csv'))
try:
...
df["ml_pred_demand"] = final_model.predict(X_all)
...
for j, mult in enumerate(multipliers):
    ...
    pred_demand = np.maximum(final_model.predict(X_cand), 0)
    cand_price  = X_test_arr[:, price_col_idx] * mult
    cost_arr    = test_df_copy["cost"].values
...
test_df_copy[[
    ...
    "price", "cost", "discounted_price", "ml_price",
